...
class Scheduler(object):
  ...
  def __init__(self, xml_net_num, sching_logto, data_over_tp):
    ...
    #
    if is_scheduler_run:
      #self.xml_parser = XMLParser("net_xmls/net_2p_stwithsingleitr.xml", str(xml_net_num))
      self.xml_parser = XMLParser("net_xmls/net_1sw_withsingletr.xml", str(xml_net_num))
    else:
      self.xml_parser = XMLParser("ext/net_xmls/net_2p_stwithsingleitr.xml", str(xml_net_num))
    self.gm = GraphMan()
    ...
    if type_ == 'join_req':
      if self.welcome_user(user_ip = src_ip,
        ...
        msg = json.dumps({'type':'join_reply',
                          'data':'welcome' })
        Scheduler.event_chief.raise_event('send_msg_to_user',msg,info_dict)
      else: #send_neg_reply to the user
        msg = json.dumps({'type':'join_reply',
                          'data':'sorry' })
        Scheduler.event_chief.raise_event('send_msg_to_user',msg,info_dict)
  ...
  def get_overudp_spwalkrule__sptprrule(self,s_id,p_id,p_walk,pitwalkbundle_dict):
    ...
    for i,node_str in list(enumerate(p_walk)):#node = next_hop
      if i == 0:
        ...
        touser_swportname = get_touser_swportname(dpid = user_info_dict['gw_dpid'],
  ...
  def test(self):
    ...
    userinfo_list = [ {'user_ip':'10.0.0.2','user_mac':'00:00:00:01:00:02','gw_dpid':1,'gw_conn_port':1},
                      {'user_ip':'10.0.0.1','user_mac':'00:00:00:01:00:01','gw_dpid':2,'gw_conn_port':2} ]
    #
    ...
    req_dict_list = [ {'data_size':1, 'slack_metric':1000, 'func_list':['f1','f2','f3'], 'parism_level':1, 'par_share':[1]},
                      ...
                      {'data_size':1, 'slack_metric':1000, 'func_list':['f1','f2','f3'], 'parism_level':1, 'par_share':[1]},
                      {'data_size':1, 'slack_metric':1000, 'func_list':['f1','f2','f3'], 'parism_level':1, 'par_share':[1]},
                      {'data_size':1, 'slack_metric':1000, 'func_list':['f1','f2','f3'], 'parism_level':2, 'par_share':[0.5, 0.5]},
                      {'data_size':1, 'slack_metric':1000, 'func_list':['f1','f2','f3'], 'parism_level':2, 'par_share':[0.5, 0.5]},
